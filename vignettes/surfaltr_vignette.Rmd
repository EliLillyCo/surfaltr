...
SurfaltR is freely available for download via Github. To install and load surfaltR, the following code can be used:
```{r, eval = FALSE}
...
As run_phobius() relies on the Phobius API, a copy of the software does not need to be downloaded on the user’s device and will isntead automatically be called by the surfaltR functions.
...
```{r, eval = FALSE}
topo <- run_phobius(AA_seq = AA_lst, fasta_file_name = "~/<your-wd>/output/AA.fasta")
```
...
```{r, eval = FALSE}
AA_lst <- get_pairs(data_file = "~/CRB1.csv", if_aa = TRUE, organism = "mouse")
mem_topo <- run_phobius(AA_seq = AA_lst, fasta_file_name = "~/<your-wd>/output/AA.fasta")
```
