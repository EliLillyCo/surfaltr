...
rank_prts <- function(counts, rank, n_prts) {
    ranks <- counts
    ranks$Position <- seq_len(nrow(ranks))
    genes <- as.data.frame(dplyr::distinct(ranks, ranks$Gene_Name))
    if (nrow(genes) == 1) {
        return(ranks)
    }
    if (rank == "length") {
        for (row in seq_len(nrow(ranks))) {
        ...
        len_genes <- genes
        len_genes$Position <- seq_len(nrow(genes))
        len_genes <- dplyr::rename(len_genes, 
                                   "Gene_Name" = "unique(ranks$Gene_Name)")
        for (row in seq_len(nrow(genes))) {
            curr_gene <- genes[row, ]
            curr_gene_trans <- subset(ranks, ranks$Gene_Name == curr_gene)
            princ_vals <- subset(curr_gene_trans, 
            substr(curr_gene_trans$APPRIS_Annotation, 1, 1) == "p")
            alt_vals <- subset(curr_gene_trans, 
            substr(curr_gene_trans$APPRIS_Annotation, 1, 1) == "a")
            alt_vals <- rbind(alt_vals, subset(
                curr_gene_trans,
            ...
            for (count in seq_len(nrow(ranks))) {
        ...
        len_genes <- len_genes[seq_along(n_prts), ]
        ...
        ord_num <- 0
        for (row in seq_len(nrow(final_ranks))) {
    ...
    if (rank == "TM") {
        ...
        for (row in seq_len(nrow(genes))) {
            ...
            a_counts <- a_trans
            for (row in seq_len(nrow(a_trans))) {
        ...
        len_genes_less <- len_genes_less[with(
            len_genes_less,
        ...
        ord_num <- 0
        for (nums in seq_len(nrow(final_ranks))) {
