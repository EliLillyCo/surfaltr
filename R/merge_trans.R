...
merge_trans <- function(princ, final_trans, if_aa){
  prim_alt_lst <- princ %>% dplyr::filter(princ$external_gene_name %in% final_trans$external_gene_name)
  final_pairs <- merge( x= final_trans, y = prim_alt_lst, by.x = "external_gene_name", by.y = "external_gene_name")
  if (if_aa == FALSE){
    final_pairs <- dplyr::filter(final_pairs, final_pairs$uniprotswissprot.x != "" |
                                   final_pairs$uniprot_isoform.x != "" |
                                   final_pairs$uniprotsptrembl.x != "")
    final_pairs <- dplyr:: filter(final_pairs, final_pairs$uniprotswissprot.y != "" |
